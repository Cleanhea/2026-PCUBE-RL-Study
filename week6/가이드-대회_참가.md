...
| 씬 오브젝트 | 역할 |
|---|---|
| `Circuit` | 트랙. 시드 하나가 서킷 하나를 결정합니다 |
| `RaceCar` | 봉인된 차량. 자식 `Agent` 가 여러분이 수정할 부분입니다 |
| `TrainingArena` | 에피소드 관리 (완주·이탈·시간초과 판정, 트랙 교체) |
| `Main Camera` | 차를 따라가는 카메라 + 계기판(속도·랩타임) |

### 트랙 바꾸기

`Circuit` 을 선택하고 인스펙터의 **[Randomize]** 를 누르면 새 서킷이 즉시 생성되고 차가 새 스타트 라인으로 옮겨집니다. **[Rebuild]** 는 현재 시드를 다시 만듭니다.
...
- **Detectable Tags**: `Track`, `OffTrack`
...
1. `Training.unity` 를 열고, `RaceCar` 의 자식 `Agent` 를 자기 프리팹으로 교체합니다.
2. 터미널에서:
